...
### 1. Парсинг модулей и проверка референсов
...
1. Прочитай все файлы из `courses/{slug}/references/`
2. Для каждого модуля определи: какие темы уроков уже покрыты референсами, а какие нет
...
**Если есть референсы:**
- Для тем, покрытых референсами: фокусируй запросы на дополнение (свежие примеры, альтернативные подходы, типичные ошибки)
- Для тем БЕЗ покрытия: стандартные запросы (полное исследование)
- Добавь в промпт субагента: "Референсы пользователя уже покрывают: {topics}. Фокусируйся на пробелах и дополнительном контексте."
